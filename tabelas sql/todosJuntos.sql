...
CREATE TABLE `sgcav`.`Missao` (
	...
	protocolo INTEGER UNSIGNED NOT NULL UNIQUE,
	...
	PRIMARY KEY (id), 
	FOREIGN KEY (idTipoMissao) REFERENCES `sgcav`.`TipodeMissao`(id)
 );
...
-- Valores interessantes
INSERT INTO `sgcav`.`Missao` (idTipoMissao, protocolo, status, nome, recursosAlocados)
